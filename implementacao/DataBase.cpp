				     ...
				     string client, string obs_client)
{
	...
	snprintf(query, 256, insert_order_store_branch_company_sql, 
			branch.c_str(), hashorder.c_str(), code, payment_mode, 
			total_itens, total_value, operator_user.c_str(),
			client.c_str(), obs_client.c_str());
